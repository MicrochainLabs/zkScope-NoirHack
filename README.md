...
- **circuits/** — Zero-knowledge circuits written in Noir.
  ...
  - `spending_permission_note | add_spending_permission_notes`: [Main circuits: enable a smart account to define permission notes that authorize an app, trading bot, or AI agent to spend ERC-20 tokens on the user's behalf: In progress]

## A new paradigm for validating transactions/user operations by the Blockchain
We’re familiar with validating transactions/user operations based on different types of signers and signing algorithms, such as  ECDSA, Passkey (WebAuthn), and multisig. Today, most accounts, including externally owned accounts(EOAs) and smart accounts, are controlled via signers. So, the Blockchain is configured to only trust the signer: it will validate and accept a transaction based on proving a valid signature.
...
## Integration
Safe account(Bybit attack, this is how can we tell the blockchain that the transction that upgrade the code that control the account is malicious and unintended behavior), smart session module, etc
new paradigm in transaction validation
## Use case
## Demo
